      ...
      map(result => {
        const items = (result.items || result.Items || []).map((item: any, index: number) => {
          ...
          const finalLng = parsedLng !== null ? parsedLng : (isThanjavur ? 79.1378 : (isBangalore ? 77.5946 : 80.2707)) + lngOffset;
            ...
            let formattedImgUrl = rawImgUrl ? (rawImgUrl.startsWith('http') || rawImgUrl.startsWith('data:') ? rawImgUrl : `https://turf-booking-backend-fixl.onrender.com${rawImgUrl.startsWith('/') ? '' : '/'}${rawImgUrl}`) : this.mockImages[index % this.mockImages.length];
  ...
  getById(id: number): Observable<Turf> {
    return this.http.get<any>(`${this.apiUrl}/${id}`).pipe(
      map((response: any) => {
        ...
        let rawImgUrl = item.imageUrl || item.ImageUrl;
        let formattedImgUrl = rawImgUrl ? (rawImgUrl.startsWith('http') || rawImgUrl.startsWith('data:') ? rawImgUrl : `https://turf-booking-backend-fixl.onrender.com${rawImgUrl.startsWith('/') ? '' : '/'}${rawImgUrl}`) : this.mockImages[0];
